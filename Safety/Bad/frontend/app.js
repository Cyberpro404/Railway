...
function updateMLOverviewHero() {
  ...
  const isIdle = zRms < 0.05 && xRms < 0.05;
  ...
  if (stateEl) stateEl.textContent = isIdle ? 'IDLE' : 'MOVING';
...
function updateMLPredictionKpi(){
  ...
  if (mlBanner) {
    ...
    if (mlBannerConf) mlBannerConf.textContent = confPercent + '%';
    if (window.lucide) lucide.createIcons();
  }
}
...
function updateMLStateCard(){
  ...
  // Determine idle vs moving from backend field (if present) or RMS threshold
  const backendState = latest.train_state;
  const zRms = Number(latest.z_rms_mm_s ?? 0);
  const xRms = Number(latest.x_rms_mm_s ?? 0);
  const idleThreshold = 0.05;

  const isIdle = backendState === 'idle' || (!backendState && zRms < idleThreshold && xRms < idleThreshold);
  ...
  trainStateEl.textContent = stateLabel;
