...
async function getHistory(seconds = 60) {
    return apiFetch(`/history?seconds=${seconds}`);
}
...
function updatePredictionDisplay() {
    ...
    // Get label info
    const info = LABEL_INFO[prediction.label] || LABEL_INFO.other_fault;
    
    // Update panel
    alertPanel.className = `alert-panel alert-${prediction.severity}`;
    ...
    if (alertConfidence) {
        alertConfidence.textContent = `Confidence: ${(prediction.confidence * 100).toFixed(1)}%`;
    }
