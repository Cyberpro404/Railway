...
let mlStats = { idle: 0, normal: 0, gap: 0, crack: 0, total: 0 };
...
function initMLInsightsCharts() {
  ...
  if (distEl) {
    chartMlDistribution = new Chart(distEl, {
      type: 'doughnut',
      data: {
        labels: ['Idle', 'Normal', 'Expansion Gap', 'Crack'],
        datasets: [{
          data: [0, 0, 0, 0],
          backgroundColor: [
            'rgba(100, 116, 139, 0.85)', // Idle - Slate 500
            'rgba(70, 230, 139, 0.85)',  // Normal - Green
            'rgba(244, 180, 0, 0.85)',   // Gap - Yellow
            'rgba(239, 68, 68, 0.85)'    // Crack - Red
          ],
          borderColor: [
            'rgba(100, 116, 139, 1)',
            'rgba(70, 230, 139, 1)',
...
async function refreshMLInsights() {
  try {
    ...
    mlStats.idle = classDist['idle'] || 0;
    mlStats.normal = classDist['normal'] || 0;
    mlStats.gap = classDist['expansion_gap'] || 0;
    mlStats.crack = classDist['crack'] || 0;
    mlStats.total = mlStats.idle + mlStats.normal + mlStats.gap + mlStats.crack;

    // Update stat cards with counts
    qs('mlIdleCount').textContent = mlStats.idle;
    ...
    const idlePct = mlStats.total > 0 ? (mlStats.idle / mlStats.total * 100).toFixed(1) : 0;
    ...
    if (chartMlDistribution) {
      chartMlDistribution.data.datasets[0].data = [mlStats.idle, mlStats.normal, mlStats.gap, mlStats.crack];
...
function renderMLPredictionsTable(predictions) {
  ...
  tbody.innerHTML = predictions.slice(0, 20).map(p => {
    // Normalize label for display and status
    const normalizedLabel = (p.label || '').toLowerCase().replace('expansion_', '');
    let labelClass = '';
    ...
    return `
      <tr>
        <td>${timestamp}</td>
        <td><span class="label-tag ${labelClass}"><i data-lucide="${statusIcon}"></i> ${(p.label || '—').replace('_', ' ').toUpperCase()}</span></td>
        <td>${confPct}%</td>
        <td>${fmt(p.z_rms, 3)}</td>
        <td>${fmt(p.x_rms, 3)}</td>
