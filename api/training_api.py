...
def setup_training_routes(app):
    ...
    @app.post("/api/training/capture-idle", tags=["Training"])
    def api_capture_idle_sample() -> Dict:
        """Capture a training sample only when the train is idle.

        Uses the latest sensor reading and enforces that `train_state` is "idle".
        Stored with label "idle" and axis "both" by default.
        """
        try:
            latest = get_db().get_latest()
            if not latest or latest == {}:
                raise HTTPException(status_code=404, detail="No sensor reading available. Please connect sensor first.")

            reading = latest.get('reading') if isinstance(latest, dict) and 'reading' in latest else latest
            if not reading:
                raise HTTPException(status_code=404, detail="Invalid sensor reading")

            train_state = reading.get("train_state") or reading.get("state")
            if train_state != "idle":
                raise HTTPException(status_code=409, detail="Train is not idle. Wait until idle to capture idle sample.")

            inserted_id, timestamp = get_training_db().insert_sample(
                reading=reading,
                axis="both",
                label="idle",
                selected_band_axis=None,
                selected_band_number=None,
            )

            return {
                "status": "ok",
                "message": "Idle training sample captured",
                "inserted_id": inserted_id,
                "timestamp": timestamp,
            ...
            logger.error(f"Idle capture error: {e}")
            raise HTTPException(status_code=500, detail="Capture-idle failed: " + str(e))
