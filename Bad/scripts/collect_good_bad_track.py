...
def main():
    ...
    try:
        while True:
            ...
            if choice == "1":
                collector.current_label = "good"
                collector.paused = False
                print("\n  >> Collecting GOOD track samples...")
                print("     (Press Enter to return to menu)")
                input()
                
            elif choice == "2":
                collector.current_label = "gap"
                collector.paused = False
                print("\n  >> Collecting EXPANSION GAP samples...")
                print("     (Press Enter to return to menu)")
                input()
                
            elif choice == "3":
                collector.current_label = "defect"
                collector.paused = False
                print("\n  >> Collecting CRACK/DEFECT samples...")
                print("     (Press Enter to return to menu)")
                input()
                
            elif choice == "4":
                collector.current_label = "idle"
                collector.paused = False
                print("\n  >> Collecting IDLE baseline samples (only when train is idle)...")
                print("     (Press Enter to return to menu)")
                input()
                ...
                if len(collector.samples) == 0:
                    ...
                    print(f"    {filepath}")
                    print(f"\n  Summary:")
                    print(f"    good:   {collector.counts['good']}")
                    print(f"    gap:    {collector.counts['gap']}")
                    print(f"    defect: {collector.counts['defect']}")
                    print(f"    idle:   {collector.counts['idle']}")
                break
